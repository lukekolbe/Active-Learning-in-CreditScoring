...
*DISCLAIMER II: code files are stored with outputs. In order to truncate outputs and save storage, the files may not have been run on full specifications, e.g. not all weight options or AL-ratios. In such cases, the specs can be changed easily within the code files for extended runs if desired.* 
